...
for (i in 1:length(m)) {
  LU_elem_crop_percent[[i]] <- readHistogramLine(m$histogram[i])
}
...
head_tmp[which(head_tmp > 2000)] <- NA
...
for (i in 1:dim(MSH)[1]) {
  ...
  Depth_per_elem[i] <- mean(CVstrat$ELV[ids] - apply(head_tmp[ ids,], 1, mean), na.rm = T)
...
for (iel in 1:length(m)) {
  ...
  for(icrop in 1:ncrops){
    ...
    dst_ml <- sqrt(apply((elem_bc[iel,] - elem_bc[elem_with_class,])^2, 1, sum))*(1/1609.34)
